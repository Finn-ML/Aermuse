    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 sm:mb-6">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            <DollarSign className="h-4 w-4 sm:h-5 sm:w-5 text-[#F7E6CA]" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-[#660033]">Key Terms</h3>
        </div>
        <span className="text-xs sm:text-sm text-[rgba(102,0,51,0.5)]">{keyTerms.length} terms identified</span>
      </div>
          ...
          return (
            ...
            >
              <button
                ...
                className="w-full p-3 sm:p-4 flex items-start gap-2 sm:gap-3 text-left hover:bg-white/30 transition-colors"
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                    <span className="font-semibold text-[#660033] text-sm sm:text-[15px]">{term.term}</span>
                    {term.section && (
                      <span
                        className="px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-[11px] rounded-full hidden sm:inline"
                    ...
                    <span
                      className="px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.05em] rounded-full"
                      style={{ background: config.bg, color: config.textColor, border: `1px solid ${config.border}` }}
                    >
                      {config.label}
                    </span>
                  </div>
                  <div className="mt-1 font-bold text-[#660033] text-sm sm:text-base">{term.value}</div>
                </div>
                <div
                  className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-transform duration-300"
                  style={{
                    background: 'rgba(102, 0, 51, 0.08)',
                    transform: isExpanded ? 'rotate(180deg)' : 'rotate(0deg)'
                  }}
                >
                  <ChevronDown className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#660033]" />
                </div>
              </button>
              ...
              >
                <div className="px-3 sm:px-4 pb-3 sm:pb-4 ml-9 sm:ml-12">
                  <p className="text-xs sm:text-sm text-[rgba(102,0,51,0.8)] leading-relaxed">
                    {term.explanation}
                  </p>
                </div>
              </div>
      ...
      {/* Legend */}
      <div className="mt-4 sm:mt-6 pt-4 sm:pt-5 border-t border-[rgba(102,0,51,0.08)] flex items-center gap-3 sm:gap-5 text-sm flex-wrap">
