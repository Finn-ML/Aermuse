/**
 * Push all email templates to Postmark as editable templates with aliases.
 *
 * Usage:
 *   npx tsx server/scripts/push-postmark-templates.ts
 *
 * Idempotent: tries to fetch each template by alias first, then creates or updates.
 */
...
if (!POSTMARK_API_KEY) {
  console.error('POSTMARK_API_KEY env var is required');
  process.exit(1);
}

const client = new postmark.ServerClient(POSTMARK_API_KEY);

// Design System Colors (same as postmark.ts)
const COLORS = {
  burgundy: '#660033',
...
// Base email template with Mustache placeholders
...
- The Aermuse Team`,
  ...
  // 7. Signature Cancelled
  {
    alias: 'signature-cancelled',
    name: 'Signature Cancelled',
...
- The Aermuse Team`,
  ...
  // 9. Proposal Notification
  {
    alias: 'proposal-notification',
    name: 'Proposal Notification',
    subject: '{{#hasContract}}New {{typeLabel}} Proposal with Contract for {{landingPageTitle}}{{/hasContract}}{{^hasContract}}New {{typeLabel}} Proposal for {{landingPageTitle}}{{/hasContract}}',
    htmlBody: emailTemplate({
      title: '{{#hasContract}}New Proposal with Contract!{{/hasContract}}{{^hasContract}}New Proposal Received!{{/hasContract}}',
          ...
          <div style="background-color: ${COLORS.cream}; border-radius: 16px; padding: 24px; margin: 20px 0;">
            <table width="100%" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td style="padding: 10px 0; color: ${COLORS.text}; font-size: 14px;">{{senderCompany}}</td>
              </tr>
              <tr>
...
Type: {{typeLabel}}
{{#hasContract}}Contract: {{contractFileName}}{{/hasContract}}
    ...
    htmlBody: emailTemplate({
      ...
      content: `${successBox('Cha-ching!')}
          <p style="margin: 20px 0; font-size: 18px; color: ${COLORS.text}; text-align: center;">
            <strong>{{buyerName}}</strong> just purchased your track:
          </p>
...
{{buyerName}} just purchased your track "{{trackTitle}}".
...
- The Aermuse Team`,
  ...
  // 12. Video Purchase Receipt
  {
    alias: 'video-purchase-receipt',
    name: 'Video Purchase Receipt',
...
- The Aermuse Team`,
  ...
  // 13. Video Sold
  {
    ...
    htmlBody: emailTemplate({
      ...
      content: `${successBox('Cha-ching!')}
          <p style="margin: 20px 0; font-size: 18px; color: ${COLORS.text}; text-align: center;">
            <strong>{{buyerName}}</strong> just purchased your video:
          </p>
...
{{buyerName}} just purchased your video "{{videoTitle}}".
...
- The Aermuse Team`,
  ...
  // 15. Split Verified
  {
    alias: 'split-verified',
    name: 'Split Verified Notification',
...
- The Aermuse Team`,
  ...
  // 16. Split Rejected
  {
    alias: 'split-rejected',
    name: 'Split Rejected Notification',
...
- The Aermuse Team`,
  ...
  // 17. All Splits Verified
  {
    alias: 'all-splits-verified',
    name: 'All Splits Verified',
...
async function main() {
  console.log(`Pushing ${templates.length} templates to Postmark...`);
  let failed = 0;
  for (const tmpl of templates) {
    try {
      await pushTemplate(tmpl);
    } catch {
      failed++;
    }
  }
  if (failed > 0) {
    console.error(`\n${failed} template(s) failed to push.`);
    process.exit(1);
  }
  console.log('Done!');
}
